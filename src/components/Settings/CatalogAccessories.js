...
const CatalogAccessories = ({ userProfile, showNotification }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const accessoryList = [];
        snapshot.forEach((doc) =>
          accessoryList.push({ ...doc.data(), id: doc.id })
        );
  ...
  return (
    <div className="max-w-lg mx-auto">
      <form onSubmit={handleAddAccessory} className="flex flex-col gap-3 mb-6">
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-gray-200 dark:divide-gray-700">
            {accessories.map(accessory => {
              const deviceType = getDeviceType(accessory.deviceTypeId);
...
export default CatalogAccessories;import React, { useState, useEffect } from 'react';
import { db } from '../../firebase';
import {
  collection,
  addDoc,

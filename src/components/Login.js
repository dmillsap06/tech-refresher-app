...
const Login = ({ onLogin, showNotification }) => {
  ...
  // Firebase Auth handles session persistence, so no localStorage logic needed
  ...
  const handleUsernameEmailSubmit = async (e) => {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        // If no profile, treat as new user and prompt for names
        setIsNewUser(true);
      }
    } catch (err) {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      // This user is an admin and group owner, so generate a groupId
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
        ...
        {!isNewUser ? (
          <form onSubmit={handleUsernameEmailSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:focus:ring-indigo-400 dark:focus:border-indigo-400"
                required
                autoComplete="email"
              />
            </div>
          ...
          <form onSubmit={handleCreateUser} className="space-y-4">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Email: <span className="font-bold text-gray-800 dark:text-gray-200">{email}</span></p>
